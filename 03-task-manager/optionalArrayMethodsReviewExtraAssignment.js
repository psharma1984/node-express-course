...
const lname = names.map((name) => name.split(" ").pop())

// - Filter names that don't match the format "<first> <last>"
const regexp = /^\w+ \w+$/;
const filteredNames = names.filter((name) => name.match(regexp))

// - Create a new array where everyone's name is converted to "Title Case"
const formatTitleCase = (name) => {
  const splitNames = name.split(" ");
  const title = splitNames.map((word) => {
    return word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()
  })
  return title.join(' ');
};

const titleCaseNames = names.map(formatTitleCase);
...
const validnames = names
  .filter((name) => name.match(regexp))       //remove names with wrong format
  .map(formatTitleCase)                   //change to title case
  .filter((name) => !name.endsWith('z'));            //last name ends with 'z'

const messages = validnames.map((name) => `${name} Please Sign Up !!!`)
